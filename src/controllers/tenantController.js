...
exports.getStep1 = async (req, res) => {
  ...
  res.render("tenant/step1", { profile });
};
exports.postStep1 = async (req, res) => {
  ...
  let p = await TenantProfile.findOne({
    where: { userId: req.session.userId },
  });
...
exports.getStep2 = async (req, res) => {
  ...
  res.render("tenant/step2", { profile });
};
...
exports.getStep3 = async (req, res) => {
  ...
  res.render("tenant/step3", { profile });
};
exports.postStep3 = async (req, res) => {
  const p = await TenantProfile.findOne({
    where: { userId: req.session.userId },
  });
  if (!p) return res.redirect("/onboarding/step2?error=profile");
  await p.update({
